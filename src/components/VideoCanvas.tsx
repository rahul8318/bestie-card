...
const drawTinyHeart = (ctx: CanvasRenderingContext2D, x: number, y: number, size: number) => {
  ctx.beginPath();
  const d = size * 1.35; // optimal scaling factor for perfect resolution
...
export const VideoCanvas = forwardRef<VideoCanvasRef, VideoCanvasProps>(
  ({ config, onTimelineUpdate, onPlaybackComplete, isFullscreen = false }, ref) => {
    ...
    const rosePetals = useRef<any[]>([]);
     ...
     // Beautiful floating friendship words that fade, merge, and dissolve
    ...
    const setupMatrixRain = (width: number) => {
      ...
      if (config.matrixChars === 'binary') {
        chars = ['0', '1'];
      } else if (config.matrixChars === 'bestie') {
        chars = ['B', 'E', 'S', 'T', 'I', 'E', 'F', 'R', 'N', 'D'];
    ...
    const sampleHeartPoints = (width: number, height: number, count: number, heartText: string = "Bestie Forever ❤️"): { x: number; y: number }[] => {
      const points: { x: number; y: number }[] = [];
      ...
      // so the bestie message is extremely sharp, readable and stunning inside the glowing outline!
      return points;
    };
    ...
    useEffect(() => {
      ...
      const render = (timestamp: number) => {
        ...
        // 1. Draw Background trail effect for joyful falling hearts
        ...
        if (isHeartSlideForBg) {
          ...
          bgGlow.addColorStop(0, 'rgba(244, 63, 94, 0.16)'); // warm pink aura
        ...
        // 2b. Draw horizontal line of joy/hearts falling down from the top (Entrance Phase)
        if (isEntrance) {
        ...
        if (stateRef.current.isPlaying) {
          ...
          if (currentIdx === -1) {
            ...
            if (stateRef.current.elapsedInSlide >= 1.5) {
            ...
            // Animation completes after centerpiece - trigger completion callback
            if (stateRef.current.elapsedInSlide >= 8.0) {
              setCurrentSlideIdx(-1);
              stateRef.current.currentSlideIdx = -1;
              stateRef.current.elapsedInSlide = 0;
              stateRef.current.globalTime = 0;
        ...
        for (let i = 0; i < pList.length; i++) {
          ...
          if (p.isSpare) {
            ...
            if (p.y > canvas.height) {
              p.y = -20;
              p.x = Math.random() * canvas.width;
               p.vy = 0.5 + Math.random() * 0.8; // extremely gentle slow gentle speed
              p.vx = (Math.random() - 0.5) * 0.3;
            }
        ...
        // --- DISSOLVING FLOATING WORDS ---
        const activeWords = floatingWords.current;
        
        // Spawn a word occasionally
        if (activeWords.length < 5 && Math.random() < 0.008) {
          const vocabulary = ['BESTIE', 'BESTFRIEND', 'BESTY', 'MUSKAN', 'MUSKAN', 'MUSKAN', 'MUSKAN', 'FOREVER', 'ALWAYS', 'FRIENDSHIP', 'DIE', 'BFF', 'BUDDY', 'PAL', 'SWEET', 'MINE', 'SMILE', 'HEART', 'HER', 'MY'];
    ...
    // Spawn joyful pink floating hearts on click/tap
    ...
    const startCanvasRecording = () => {
      ...
      try {
        ...
        recorder.onstop = () => {
          ...
          a.href = url;
           a.download = `bestie_particles_${Date.now()}.webm`;
